...
def load_words(textfile_name: str):
    ...
    textfile = open(f"Probability/{textfile_name}", "r")
    wordlist = []
...
def dictionary_build(wordlist: list, next: bool):
        ...
        for index in range(length):
            ...
            for cut in range(len(previous) + 1):
                ...
                if next:
                    ...
                    data[length, index, a, letter] += 1
    return data


def dataframe_build(data: dict, next: bool):
...
def build_dataframe(output_name: str, textfile_name: str = None, next: bool = True):
    ...
    word_dictionary = dictionary_build(wordlist, next)
    dataframe = dataframe_build(word_dictionary, next)
    if not output_name.endswith(".csv"):
        output_name += ".csv"
    print(f"Saving Dataframe: {output_name}")
    dataframe.to_csv(f"Probability/{output_name}", index=False)
...
def load_CSV(filename: str):
    ...
    if not filename.endswith(".csv"):
        filename += ".csv"
    return read_csv(f"Probability/{filename}")
...
word_analysis([3], [1], ["t"], [], "words_long")